...
if ( empty($addresses) ) {
    echo '<p>'. __('No address on file. Please add one below.', permaslug()) .'</p>';
} else {
...
?>
    <h2><?php _e('Edit address', permaslug()); ?></h2>
<?php else: ?>
    <h2><?php _e('Add a new address', permaslug()); ?></h2>
    <?php if ( !isset( $_GET['ref'] ) ): ?>
    <p>
        <a href="#" class="button btn-import-billing" style="display: none;"><?php _e('Import billing address', permaslug()); ?></a>
        <a href="#" class="button btn-import-shipping" style="display: none;"><?php _e('Import shipping address', permaslug()); ?></a>
    </p>
    <?php endif; ?>
<?php endif; ?>
...
<form action="" method="post" class="wcms-address-form">
    ...
    <?php if ( $address_id > -1 ): ?>
        <input type="submit" class="button alt" id="use_address" value="<?php _e('Update Address', permaslug()); ?>" />
    <?php else: ?>
        <input type="submit" class="button alt" id="use_address" value="<?php _e('Save Address', permaslug()); ?>" />
    <?php endif; ?>
