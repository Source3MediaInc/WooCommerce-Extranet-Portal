<form method="post" action="" id="address_form">
    <div id="address_wrapper">
        ...
        <div id="cart_items">
            <h2><?php _e('Cart Items', permaslug()); ?></h2>
            <ul class="cart-items">
                <?php
            ...
            ?>
            <a class="duplicate-cart-button user-duplicate-cart" href="#"><?php _e('Duplicate Cart', permaslug()); ?></a>
        ...
        <div id="cart_addresses">
            ...
            ?>
            <h2>
                <?php _e('Saved Addresses', permaslug()); ?>
                <a class="h2-link" href="<?php echo $add_url; ?>"><?php _e('Add New', permaslug()); ?></a>
            </h2>
            ...
            <div id="addresses_container">
		                ...
		                <div class="account-address">
                            ...
                            if ( isset($addr['default_address']) && $addr['default_address'] ) {
                                if (function_exists('wc_get_endpoint_url'))
                                    echo '<a href="'. wc_get_endpoint_url( 'edit-address', 'shipping', get_permalink(wc_get_page_id( 'myaccount' )) ) .'" class="edit">'. __( 'edit', permaslug() ) .'</a>';
                                else
                                    echo '<a href="'. esc_url( add_query_arg('address', 'shipping', get_permalink(wc_get_page_id( 'edit_address' ) ) ) ) .'" class="edit">'. __( 'edit', permaslug() ) .'</a>';
                            } else {
                                echo '<a class="edit" href="'. $addresses_url .'#shipping_address_'. $x .'">'. __('edit', permaslug()).'</a>';
                            }
                            ?>
		                    ...
		                    <ul class="items-column" id="items_column_<?php echo $x; ?>">
		                        ...
		                        <li data-product-id="<?php echo $value['product_id']; ?>" data-key="<?php echo $key; ?>" class="address-item address-item-<?php echo $value['product_id']; ?> address-item-key-<?php echo $key; ?>">
		                            ...
		                            <?php endfor; ?>
		                            <a class="remove" href="#"><img style="width: 16px; height: 16px;" src="<?php echo plugins_url('images/delete.png', self::FILE); ?>" class="remove" title="<?php _e('Remove', permaslug()); ?>"></a>
		                        </li>
		                        <?php
		                                }
		                            }
		                        ...
		                        <?php else: ?>
		                        <li class="placeholder"><?php _e('Drag items here', permaslug()); ?></li>
		                        <?php endif; ?>
		                    </ul>
		                </div>
		                <?php
                ...
                ?>
            <h2>
                <?php _e('Shipping Addresses', permaslug()); ?>
            <a class="button" href="?address-form"><?php _e('Add New', permaslug()); ?></a>
            </h2>
    ...
    <div class="form-row">
        ...
        <input type="submit" name="set_addresses" value="<?php echo __('Save Addresses and Continue', permaslug()); ?>" class="button alt" />
    </div>
</form>
<?php if ( $user->ID == 0 ): ?>
...
<div id="duplicate_address_form_template" style="display:none;">
    <form id="duplicate_cart_form">
        ...
        <p>
            <select name="address_id">
                <option value=""><?php _e('Select an existing address', permaslug()); ?></option>
                <?php
    ...
    <input type="submit" id="duplicate_cart" class="button" value="<?php _e('Duplicate Cart Items', permaslug()); ?>" />
    </form>
</div>
